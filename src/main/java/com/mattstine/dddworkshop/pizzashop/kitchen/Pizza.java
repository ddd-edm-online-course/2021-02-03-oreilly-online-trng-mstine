...
public final class Pizza implements Aggregate {
    ...
    void finishPrep() {
        if (state != State.PREPPING) {
            throw new IllegalStateException("can only finishPrep() from State.PREPPING");
        }
    ...
    void startBake() {
        if (state != State.PREPPED) {
            throw new IllegalStateException("can only startBake() from State.PREPPED");
        }
    ...
    void finishBake() {
        if (state != State.BAKING) {
            throw new IllegalStateException("can only finishBake() from State.BAKING");
        }
